...
def _load_company_functions():
    from engine.extractors.market_universe import fetch_sector, kospi_kosdaq_corp_list
...
def _get_kr_normalized_sector_and_issuer() -> pd.DataFrame:
    ...
    market_df["stock_code"] = market_df["stock_code"].map(normalize_kr_stock_code)
    ...
    return pd.DataFrame(
        {
            "issuer_id": stock_codes.map(lambda code: issuer_id_of(code, KR_MARKET_CONFIG)),
...
def _get_kr_normalized_security_master() -> pd.DataFrame:
    ...
    return pd.DataFrame(
        {
            "security_id": stock_codes.map(lambda code: security_id_of(code, KR_MARKET_CONFIG)),
            "issuer_id": stock_codes.map(lambda code: issuer_id_of(code, KR_MARKET_CONFIG)),
