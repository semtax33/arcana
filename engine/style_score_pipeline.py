    ...
    *,
    ...
    for security_id, group in frame.groupby("security_id", sort=False):
        ...
        score_row: dict[str, Any] = {
            ...
            "stock_code": _string_or_empty(first.get("stock_code"))[:6],
...
def _format_factor_score_frame(frame: pd.DataFrame, trade_date: date) -> pd.DataFrame:
    ...
    for column in ["issuer_id", "stock_code", "company_name", "industry_schema"]:
        if column not in result.columns:
            result[column] = ""
        result[column] = result[column].fillna("").astype(str)
    result["stock_code"] = result["stock_code"].str.slice(0, 6)
