...
const FINANCIAL_DIR = path.join(ROOT, "data-lake", "silver", "dart", "normalized");
const PRICE_PATH = path.join(ROOT, "data-lake", "silver", "krx", "price", "normalized_price.csv");
const SHARES_PATH = path.join(ROOT, "data-lake", "silver", "krx", "shares", "normalized_shares.csv");
...
const FACTORS = [
  "at", "seq", "ceq", "ppent", "act", "lct", "invt", "rect", "ap", "dltt", "dlc", "che",
  "retained_earnings", "sale", "ni", "ni_parent", "oiadp", "oibdp", "cogs", "dp", "xrd",
  "xint", "oancf", "capx", "fcf", "fcff", "fcfe", "ffo", "dvpsp", "dvpsx", "sstk", "prstkc",
  "eps", "bps", "sps", "cps", "csho", "mcap_mil", "gpm", "opm", "ebitda_margin", "npm", "tax_rate", "nopat", "roe",
  "avg_parent_equity", "roa", "iroe", "roic_financial", "roic_operational", "asset_turnover",
  "receivables_turnover", "inventory_turnover", "inv_days", "ar_days", "ap_days", "ccc",
  "working_capital", "wc_to_sales_pct", "working_capital_turnover", "sales_yoy_pct", "op_yoy_pct",
  "sales_change_mil", "op_change_mil", "rdsr_pct", "eps_yoy_pct", "asset_yoy_pct", "cfo_yoy_pct",
  "fcf_yoy_pct", "ffo_yoy_pct", "peg", "epr", "bpr", "tpr", "spr", "cpr", "fcfpr", "npr",
  "rpr", "ebitda_to_ev", "ev_to_ebitda", "ev_to_nopat", "na_5", "na_20", "na_50", "na_150",
  "na_200", "tr_12_1", "tr_6_1", "tr_3_1", "ret_1m", "high52w_gap_pct", "risk_adj_mom",
  "vol_12_1_ann", "mdd1yr_12_1_pct", "adturn_pct_12_1", "net_debt_to_ebitda", "net_debt_to_ocf",
  "fc_to_ndr", "icr_times", "interest_coverage", "current_ratio", "debt_to_equity", "cash_to_debt",
  "sharehold_div_yield", "sharehold_net_buyback_yield", "sharehold_return", "tdpr", "per", "pbr",
  "pcr", "psr", "roce", "total_interest_coverage", "debt_ratio", "dividend_yield", "payout_ratio",
  "altman_z_score", "beneish_m_score", "f_score",
];
...
function extractAmountByName(rows, patterns, statementTypes, absoluteValue = false) {
  const values = [];
...
function financialFileMeta(name) {
  ...
  return { stockCode: match[1], year: Number(match[2]), month: Number(match[3]) };
}
...
function discoverAnnualFiles() {
  ...
  for (const name of fs.readdirSync(FINANCIAL_DIR)) {
    ...
    byStock.get(meta.stockCode).push({ ...meta, path: path.join(FINANCIAL_DIR, name) });
  }
...
function readAnnualFinancials(stockCode, files) {
  ...
  for (const file of files) {
    ...
    values.RETAINED_EARNINGS_FALLBACK = extractAmountByName(csvRows, [/이익\s*잉여금/u, /결손금/u], ["BS"]);
    values.LONG_TERM_DEBT_FALLBACK = extractAmountByName(csvRows, [/장기\s*차입/u, /사채/u], ["BS"], true);
    values.INTEREST_EXPENSE_FALLBACK = extractAmountByName(csvRows, [/이자\s*비용/u], ["CF", "IS", "CIS"], true);
    values.INTEREST_PAID_FALLBACK = extractAmountByName(csvRows, [/이자\s*지급/u], ["CF", "IS", "CIS"], true);
    values.FINANCE_COST_FALLBACK = extractAmountByName(csvRows, [/금융\s*비용/u], ["CF", "IS", "CIS"], true);
    values.stock_code = stockCode;
    values.security_id = securityId(stockCode);
    values.fiscal_year = file.year;
    values.financial_period_ts = Date.parse(`${file.year}-12-31T00:00:00+09:00`);
    rows.push(values);
  }
  return addAnnualFinancialFactors(rows.sort((a, b) => a.financial_period_ts - b.financial_period_ts));
}
...
function addAnnualFinancialFactors(rows) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    r.rect = first(source, "TRADE_RECEIVABLES", "TRADE_AND_OTHER_RECEIVABLES", "OTHER_RECEIVABLES");
    r.ap = first(source, "TRADE_PAYABLES", "TRADE_AND_OTHER_PAYABLES", "OTHER_PAYABLES");
    r.dltt = first(source, "LONG_TERM_DEBT", "LONG_TERM_DEBT_FALLBACK");
    r.dlc = source.SHORT_TERM_DEBT ?? null;
    r.che = fill0(source.CASH_AND_EQUIVALENTS) + fill0(source.SHORT_TERM_FINANCIAL_ASSETS);
    ...
    r.xint = first(source, "INTEREST_EXPENSE_FALLBACK", "INT_PAID", "INTEREST_PAID_FALLBACK", "FINANCE_COST_FALLBACK");
    ...
    if (!isCovered(r.dp)) r.dp = fill0(source.DEPRECIATION_EXPENSE) + fill0(source.AMORTIZATION);
    ...
    r.avg_assets = isCovered(r.at) && isCovered(prev.at) ? (r.at + prev.at) / 2 : null;
    r.avg_equity = isCovered(r.seq) && isCovered(prev.seq) ? (r.seq + prev.seq) / 2 : null;
    r.avg_inventory = isCovered(r.invt) && isCovered(prev.invt) ? (r.invt + prev.invt) / 2 : null;
    r.avg_receivables = isCovered(r.rect) && isCovered(prev.rect) ? (r.rect + prev.rect) / 2 : null;
    r.avg_payables = isCovered(r.ap) && isCovered(prev.ap) ? (r.ap + prev.ap) / 2 : null;
    ...
    if (isCovered(r.tax_rate) && (r.tax_rate < 0 || r.tax_rate > 1)) r.tax_rate = null;
    r.nopat = mul(r.oiadp, isCovered(r.tax_rate) ? 1 - r.tax_rate : null);
    r.avg_parent_equity = isCovered(r.ceq) && isCovered(prev.ceq) ? (r.ceq + prev.ceq) / 2 : null;
    r.roe = div(r.ni_parent, r.avg_parent_equity);
    r.roa = div(r.ni, r.avg_assets);
    r.iroe = div(add(r.ni_parent, fill0(r.xrd) * (1 - fill0(r.tax_rate))), r.avg_parent_equity);
    r.debt = fill0(r.dltt) + fill0(r.dlc);
    r.net_debt = r.debt - fill0(r.che);
    r.invested_capital_financial = isCovered(r.seq) ? r.seq + r.debt - fill0(r.che) : null;
    r.invested_capital_operational = fill0(r.rect) + fill0(r.invt) - fill0(r.ap) + fill0(r.ppent) + fill0(source.INTANGIBLE_ASSETS);
    r.avg_ic_financial = isCovered(r.invested_capital_financial) && isCovered(prev.invested_capital_financial) ? (r.invested_capital_financial + prev.invested_capital_financial) / 2 : null;
    r.avg_ic_operational = isCovered(r.invested_capital_operational) && isCovered(prev.invested_capital_operational) ? (r.invested_capital_operational + prev.invested_capital_operational) / 2 : null;
    ...
    r.ccc = isCovered(r.inv_days) && isCovered(r.ar_days) && isCovered(r.ap_days) ? r.inv_days + r.ar_days - r.ap_days : null;
    ...
    const wcChange = isCovered(r.working_capital) && isCovered(prev.working_capital)
    ...
    r.fcfe = isCovered(r.fcf)
      ? r.fcf + fill0(r.debt_issue) - fill0(r.debt_repay) + fill0(r.sstk) - fill0(r.prstkc)
      : null;
    r.sales_yoy_pct = pctYoy(r.sale, prev.sale);
    r.op_yoy_pct = pctYoy(r.oiadp, prev.oiadp);
    r.sales_change_mil = isCovered(r.sale) && isCovered(prev.sale) ? (r.sale - prev.sale) / 1_000_000 : null;
    r.op_change_mil = isCovered(r.oiadp) && isCovered(prev.oiadp) ? (r.oiadp - prev.oiadp) / 1_000_000 : null;
    ...
    r.interest_coverage = div(r.oiadp, isCovered(r.xint) ? Math.abs(r.xint) : null);
    r.current_ratio = div(r.act, r.lct);
    r.debt_to_equity = div(r.debt, r.seq);
    r.cash_to_debt = div(r.che, r.debt);
    r.retained_earnings = first(source, "RETAINED_EARNINGS", "RETAINED_EARNINGS_FALLBACK");
    r.altman_z_score = add(add(add(
...
function beneish(r, prev, sgna) {
  ...
  const gmi = div(div(sub(prev.sale, prev.cogs), prev.sale), div(sub(r.sale, r.cogs), r.sale));
  const aqi = div(sub(1, div(add(r.act, r.ppent), r.at)), sub(1, div(add(prev.act, prev.ppent), prev.at)));
  const sgi = div(r.sale, prev.sale);
  const depi = div(div(prev.dp, add(prev.ppent, prev.dp)), div(r.dp, add(r.ppent, r.dp)));
  const sgai = div(div(sgna, r.sale), div(prev.SGNA ?? null, prev.sale));
  const lvgi = div(div(add(r.dltt, r.dlc), r.at), div(add(prev.dltt, prev.dlc), prev.at));
...
function piotroski(r, prev) {
  ...
  score += isCovered(r.debt_to_equity) && isCovered(prev.debt_to_equity) && r.debt_to_equity < prev.debt_to_equity ? 1 : 0;
  score += isCovered(r.current_ratio) && isCovered(prev.current_ratio) && r.current_ratio > prev.current_ratio ? 1 : 0;
  score += fill0(r.sstk) <= 0 ? 1 : 0;
  score += isCovered(r.gpm) && isCovered(prev.gpm) && r.gpm > prev.gpm ? 1 : 0;
  score += isCovered(r.asset_turnover) && isCovered(prev.asset_turnover) && r.asset_turnover > prev.asset_turnover ? 1 : 0;
  return score;
}
...
function getNetIncomeForPayout(financialByStock, stockCode, year) {
  ...
  if (payoutNetIncomeCache.has(cacheKey)) return payoutNetIncomeCache.get(cacheKey);
  const file = (financialByStock.get(stockCode) ?? []).find((item) => item.year === year);
  if (!file) {
  ...
  const value = matched ? normalizeNumericAmount(matched.normalized_amount) : null;
...
async function loadGroupedCsv(filePath, wantedSecurities, columns) {
  ...
  for await (const line of rl) {
    ...
    if (lineNo % 1_000_000 === 0) {
      process.stderr.write(`[INFO] read ${path.basename(filePath)} lines=${lineNo.toLocaleString()}\n`);
    }
  }
  return grouped;
}
...
function rollingStd(values, end, window, minPeriods) {
  const xs = [];
  const start = Math.max(0, end - window + 1);
  for (let i = start; i <= end; i++) if (isCovered(values[i])) xs.push(values[i]);
...
function addDailyFactors(rows) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (isCovered(r.altman_z_score)) {
      r.altman_z_score = add(r.altman_z_score, mul(0.6, div(r.market_cap, r.TOTAL_LIABILITIES ?? null)));
    }
    ...
    r.enterprise_value = isCovered(r.market_cap) ? r.market_cap + fill0(r.debt) - fill0(r.che) : null;
    ...
    r.sharehold_net_buyback_yield = mul(div(fill0(r.prstkc) - fill0(r.sstk), r.market_cap), 100);
    r.sharehold_return = fill0(r.sharehold_div_yield) + fill0(r.sharehold_net_buyback_yield);
    ...
    const windowReturns = [];
    for (let j = Math.max(0, i - 231 + 1); j <= i; j++) windowReturns.push(shiftedRet[j]);
    const validReturns = windowReturns.filter(isCovered);
    r.mdd1yr_12_1_pct = validReturns.length >= 60 ? maxDrawdown(windowReturns) * 100 : null;
...
function mergeStockRows(stockCode, priceRows, sharesRows, financialRows, financialByStock) {
  ...
  const years = [...new Set(priceRows.map((r) => Number(String(r.trade_date).slice(0, 4))))];
  const { dvpsx, tdpr } = dividendMaps(financialByStock, stockCode, years);
  const rows = [];
  let shareIdx = -1;
  let finIdx = -1;
  const financialAsofRows = financialRows.map((r) => ({ ...r, _ts: r.financial_period_ts }));
...
async function main() {
  ...
  const wantedSecurities = new Set([...financialByStock.keys()].map(securityId));
  process.stderr.write(`[INFO] annual-financial stocks=${financialByStock.size.toLocaleString()}\n`);

  const [priceGroupedRaw, shareGroupedRaw] = await Promise.all([
    loadGroupedCsv(PRICE_PATH, wantedSecurities, ["security_id", "trade_date", "close", "volume"]),
    loadGroupedCsv(SHARES_PATH, wantedSecurities, ["security_id", "trade_date", "shares", "market_cap"]),
  ]);
  ...
  for (const [stockCode, files] of financialByStock.entries()) {
    ...
    try {
      ...
      const rows = mergeStockRows(stockCode, priceRows, shareRows, financialRows, financialByStock);
      for (const row of rows) {
        rowCount++;
        for (const factor of FACTORS) {
          if (isCovered(row[factor])) coverage.set(factor, coverage.get(factor) + 1);
        }
      }
      processedStocks++;
      if (processedStocks % 100 === 0) {
        process.stderr.write(`[INFO] processed stocks=${processedStocks.toLocaleString()} rows=${rowCount.toLocaleString()}\n`);
      }
    } catch (error) {
      errors.push({ stockCode, error: String(error && error.stack ? error.stack : error) });
    }
  }
  ...
  const rows = FACTORS.map((factor) => {
    ...
    return { factor, row_count: rowCount, covered_count: covered, missing_count: missing, coverage_ratio: ratio, coverage_pct: ratio * 100 };
  }).sort((a, b) => a.coverage_ratio - b.coverage_ratio || a.factor.localeCompare(b.factor));
  ...
  process.stdout.write(JSON.stringify({ summary, csv_path: OUT_CSV, summary_path: OUT_SUMMARY, lowest: rows.slice(0, 15), highest: rows.slice(-15).reverse() }, null, 2));
}
