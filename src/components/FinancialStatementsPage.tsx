...
const statementViews: Record<FinancialStatementCode, StatementView> = {
  ...
  BS: {
    code: "BS",
    title: "재무상태표 (Balance Sheet)",
    icon: WalletCards,
  },
  CF: {
    code: "CF",
    title: "현금흐름표 (Cash Flow)",
...
const sampleFinancials: FinancialStatementData[] = [
  {
    ...
    accounts: [
      ...
      createSampleAccount("cogs", "매출원가(COGS)", "IS", [21.4, 22.0, 24.2, 25.6, 23.8, 29.4, 34.8, 36.5, 37.7, 46.5]),
  ...
  },
  {
    code: "CF",
    accounts: [
      createSampleAccount("operating_cf", "영업CF", "CF", [17.1, 15.0, 18.7, 28.6, 16.8, 23.4, 29.8, 32.7, 41.0, 44.8]),
      createSampleAccount("investing_cf", "투자CF", "CF", [4.8, -9.7, -34.1, -25.2, 3.5, -13.1, -15.1, -14.2, -31.2, -33.1]),
      createSampleAccount("financing_cf", "재무CF", "CF", [0, 0.01, 0.01, 1.6, -0.22, -2.8, -2.5, -1.3, -0.17, 1.2]),
      createSampleAccount("free_cf", "FCF", "CF", [10.0, 7.3, 12.3, 22.9, 8.4, 11.8, 14.9, 19.3, 28.8, 27.2]),
      createSampleAccount("capex", "설비투자", "CF", [null, null, null, null, null, null, null, null, null, null]),
...
function normalizeTablePoints(account: FinancialAccountSeries, period: FinancialApiPeriod) {
  ...
  const displayValues = labels.map((_, index) => points[index]?.displayValue ?? formatValue(values[index]));
    ...
    <div className="financial-modal-backdrop" role="presentation" onClick={onClose}>
      ...
      >
        ...
        <div className="financial-modal-stats">
          <article>
            <span>최신값</span>
            <strong>{metric.displayValues[metric.displayValues.length - 1] ?? formatValue(latest)}</strong>
          </article>
          <article>
            <span>최대값</span>
...
export function FinancialStatementsPage({ stockCode }: FinancialStatementsPageProps) {
  const [period, setPeriod] = useState<FinancialApiPeriod>("annual");
  ...
  const handleMetricSelect = (statement: StatementView, account: FinancialAccountSeries) => {
        ...
        setSelectedMetric((current) =>
          current?.account.canonicalId === account.canonicalId
            ? {
                ...current,
                isLoading: false,
                errorMessage: `${error.message} 표 데이터를 대신 표시합니다.`,
              }
            : current,
        );
      });
  ...
  return (
    <section className="financial-page">
      <nav className="financial-subnav" aria-label="재무 하위 메뉴">
        ...
        {["재무비율", "적정가치", "실적", "애널리스트", "배당", "공시"].map((item) => (
      ...
      <div className="financial-period-row">
        <span>기간:</span>
        {(Object.keys(periodLabels) as FinancialApiPeriod[]).map((item) => (
          <button
      ...
      {errorMessage && <p className="financial-api-note">{errorMessage}</p>}

      <div className="financial-statement-stack">
        {visibleSections.map((statement) => {
          ...
          return (
            <section className="financial-statement-card" key={statement.code}>
                    ...
                    {statement.accounts.map((account) => {
                      ...
                      return (
                        <tr
                          ...
                          onClick={() => handleMetricSelect(statement, account)}
                        >
